...
class KNearestNeighbor():    
    ...
    def predict(self, features, ignore_first=False):
        ...
        size = len(features)
        for i in range(size):
            index_distance = np.argsort(distance[i])
            neighbors_list =[]
            for j in range(self.n_neighbors):
                neighbors_list.append(self.targets[index_distance[j]])
                if ignore_first == True:
                    neighbors_list=neighbors_list[1:]

            if self.aggregator == "mean":
                result.append(np.mean(neighbors_list, axis = 0))
            if self.aggregator == "median":
                result.append(np.median(neighbors_list, axis = 0))
            if self.aggregator == "mode":
                temp_size = len(neighbors_list[0])
                temp = np.zeros(temp_size)
                for i in range(temp_size):
                    temp_p = []
                    for j in range(len(neighbors_list)):
                        temp_p.append(neighbors_list[j][i])
                    most_requent_num= mode(temp_p)
                    temp[i] = most_requent_num
                result.append(temp)
