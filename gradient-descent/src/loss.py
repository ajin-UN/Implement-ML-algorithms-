    ...
    def forward(self, X, w, y):
        ...
        Returns:
            loss - (float) The calculated loss normalized by the number of
                examples, N.
        """
        loss = np.mean(0.5 * np.square(np.subtract(y, np.array([np.dot(w.T, x) for x in X]))))
        return loss + self.regularization.forward(w) if self.regularization else loss
    ...
    def backward(self, X, w, y):
        ...
        Returns:
            gradient - (np.array) The (d+1)-dimensional gradient of the loss
                ...
                the bias term.
        """
        sub = np.subtract(y, np.array([np.dot(w.T, x) for x in X]))
        gradient = np.mean(np.array([X[i] * sub[i] for i in range(X.shape[0])]), 0) * (-1)
        return np.add(gradient, self.regularization.backward(w)) if self.regularization else gradient
    ...
    def backward(self, X, w, y):
        ...
        for i in range(X.shape[0]):
            if cur_val[i] > 0:
                gradient -= y[i] * X[i]
        gradient /= X.shape[0]
        return gradient + self.regularization.backward(w) if self.regularization else gradient
