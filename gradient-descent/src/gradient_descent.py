...
class GradientDescent:
    ...
    def fit(self, features, targets, batch_size=None, max_iter=1000):
        ...
        for i in range(max_iter):
            ...
            pre_loss = self.loss.forward(batch, w, batch_target)

            w -= self.loss.backward(batch, w, batch_target) * self.learning_rate

            loss = self.loss.forward(batch, w, batch_target)

            if np.abs(loss - pre_loss) < 1e-4:
                self.model = w
                return
    ...
    def predict(self, features):
        ...
        return np.where(confidence < 0, -1, 1)
    ...
    def confidence(self, features):
        ...
        Returns:
            confidence - (np.array) A 1D array of confidence values of length
                ...
                features.
        """
        return np.dot(np.c_[features, np.ones((features.shape[0], 1))], self.model)