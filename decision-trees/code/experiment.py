...
def run(data_path, learner_type, fraction):
    ...
    features, targets, names = load_data(data_path)
    train_features, train_targets, test_features, test_targets = train_test_split(features, targets, fraction)

    model = PriorProbability() if learner_type == "prior_probability" else DecisionTree(names)
    model.fit(train_features, train_targets)

    predictions = model.predict(test_features)
    res = confusion_matrix(test_targets, predictions)
    acc = accuracy(test_targets, predictions)
    pre, rec = precision_and_recall(test_targets, predictions)
    f1 = f1_measure(test_targets, predictions)
    return res, acc, pre, rec, f1