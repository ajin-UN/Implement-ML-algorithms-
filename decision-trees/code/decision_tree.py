...
class DecisionTree():
    ...
    def fit(self, features, targets):
        ...
        Args:
            ...
            targets (np.array): numpy array containing class labels for each of the N
                examples.
        """
        def helper(fs, ts, names, tre):
            

            if len(np.unique(ts)) == 1:
                return Tree(ts[0])
            elif len(fs) == 0 or len(fs[0]) == 0:
                return Tree(tre)

            label, max_gain = 0, 0
            features_right, features_left, targets_right, targets_left = [], [], [], []

            for i in range(len(fs[0])):
                gain = information_gain(fs, i, ts)
                if gain > max_gain:
                    max_gain = gain
                    label = i
            for i in range(len(ts)):
                if fs[i][label] == 1:
                    targets_right.append(ts[i])
                    features_right.append(list(fs[i])[:label] + list(fs[i])[label + 1:])
                else:
                    targets_left.append(ts[i])
                    features_left.append(list(fs[i])[:label] + list(fs[i])[label + 1:])

            temp = [helper(np.array(features_left),
                                np.array(targets_left),
                                names[:label] + names[label + 1:],
                                np.argmax(np.bincount(ts))),
                     helper(np.array(features_right),
                                np.array(targets_right),
                                names[:label] + names[label + 1:],
                                np.argmax(np.bincount(ts)))]
            return Tree(None, names[label], self.attribute_names.index(names[label]), temp)
        
        targets = np.array(list(map(int, targets)))
        self.tree = helper(features, targets, self.attribute_names, np.argmax(np.bincount(targets)))
    ...
    def predict(self, features):
        ...
        def predict_helper(feature, node):
            if node.value is not None:
                return node.value
            if feature[node.attribute_index]:
                return predict_helper(feature, node.branches[1])
            else:
                return predict_helper(feature, node.branches[0])
        
        for feature in features:
            result.append(predict_helper(feature, self.tree))
...
def information_gain(features, attribute_index, targets):
    ...
    """
    size = len(targets)
    categorials, freq = np.unique(targets, return_counts=True)
    prob_true = freq[0]/size

    entropy = -prob_true * np.log2(prob_true) - (1 - prob_true) * np.log2(1 - prob_true)
    ...
    probability1 = sum1/size
    probability2 = sum2/size

    if sum1 > 0:
        prob1 = count1/sum1
        entropy1 = -prob1 * np.log2(prob1) - (1-prob1) * np.log2(1-prob1)
    if sum2 > 0:
        prob1 = count2/sum2
        entropy2 = -prob1 * np.log2(prob1) - (1-prob1) * np.log2(1-prob1)

    result = entropy -  (probability1 * entropy1) - (probability2 * entropy2)
