...
def run_model(model,running_mode='train', train_set=None, valid_set=None, test_set=None, 
	...
	if running_mode == "train":
		...
		if valid_set:
			valid_data_loader = DataLoader(valid_set, batch_size=batch_size, shuffle=shuffle)
		loss = {"train": [], "valid": []}
		acc = {"train": [], "valid": []}
		pre_train_loss = 0.0
		for i in range(n_epochs):
			model, train_loss, train_accuracy = _train(model, train_data_loader, optimizer)
			loss["train"].append(train_loss)
			acc["train"].append(train_accuracy)
			if valid_set:
				valid_loss, valid_accuracy =_test(model, valid_data_loader)
				loss["valid"].append(valid_loss)
				acc["valid"].append(valid_accuracy)
			if abs(pre_train_loss - train_loss) < stop_thr:
				break
			pre_train_loss = train_loss
		return model, loss, acc

	else:
		test_data_loader = DataLoader(test_set, batch_size = batch_size, shuffle=shuffle)
		return _test(model, test_data_loader)
...
def _train(model,data_loader,optimizer,device=torch.device('cpu')):
	...
	for i , data in enumerate(data_loader):
		...
		total_holder = total_holder + labels.size(0)
		true_holder += (predicted == labels).sum().item()
	train_accuracy = true_holder / total_holder * 100
	train_loss /= (i + 1)
...
def _test(model, data_loader, device=torch.device('cpu')):
	...
	with torch.no_grad():
		for i, data in enumerate(data_loader):
			...
			test_loss += loss.item()
			_, predicted = torch.max(outputs.data, 1)
			total_holder += labels.size(0)
			true_holder += (predicted == labels).sum().item()
	test_accuracy = true_holder / total_holder * 100
	test_loss /= (i + 1)
