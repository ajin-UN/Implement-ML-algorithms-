...
class GMM():
    ...
    def _e_step(self, features):
        ...
        for i in range(self.n_clusters):
            temp = self._posterior(features, i).reshape(-1, 1)
            posterior_probability = temp if i == 0 else np.c_[posterior_probability, temp]
    ...
    def _m_step(self, features, assignments):
        ...
        for i in range(self.n_clusters):
            means[i] = features.T.dot(assignments[:, i]) / np.sum(assignments[:, i])
            mixing_weights[i] = np.sum(assignments[:, i]) / features.shape[0]
            temp = ((features - means[i]) ** 2).T.dot(assignments[:, i]) / np.sum(assignments[:, i])
            covariances[i] = temp if self.covariance_type == 'diagonal' else np.mean(temp)
    ...
    def _log_likelihood(self, features, k_idx):
        ...
        """
        result = np.log(self.mixing_weights[k_idx]) + multivariate_normal.logpdf(features, self.means[k_idx], self.covariances[k_idx])
        return result
