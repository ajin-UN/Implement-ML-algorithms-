...
class MultiArmedBandit:
    ...
    def fit(self, env, steps=100000):
        ...
        for i in range(steps):
            state = observation
            action = env.action_space.sample() if np.random.rand() < self.epsilon else np.argmax(state_action_values[state,:])
            observation, reward, done, info = env.step(action)

            counts[action] += 1
            state_action_values[state][action] += (1 / counts[action]) * (reward - state_action_values[state][action])

            sum_rewards += reward
            if i % s == 0 and i != 0:
                rewards[int(i//s) - 1] = sum_rewards / s
                sum_rewards = 0
