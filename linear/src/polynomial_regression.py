...
class PolynomialRegression():
    ...
    def fit(self, features, targets):
        ...
        degree = self.degree
        size = len(features)
        x_matrix = np.zeros((size, degree + 1))

        for i in range(size):
            for j in range(degree + 1):
                x_matrix[i][j] = np.power(features[i], j)

        x_matrix_t = np.transpose(x_matrix)
        dot_product = np.dot(x_matrix_t, x_matrix)
        inverse_dot_product = np.linalg.inv(dot_product) 
        dot_product_y = np.dot(inverse_dot_product, x_matrix_t)

        model = np.dot(dot_product_y, targets)
    ...
    def predict(self, features):
        ...
        for i in range(size):
            ...
            for j in range(1, self.degree + 1):
                temp = temp + self.model[j] * np.power(features[i], j)
            predictions[i] = temp
        return predictions
