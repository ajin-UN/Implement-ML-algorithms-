...
def generate_regression_data(degree, N, amount_of_noise=1.0):
    ...
    c = np.random.uniform(low=-10, high=10, size=degree+1)
    y = np.zeros(N)
    
    for i in range(N):
        for j in range(degree + 1):
            y[i] += np.power(x[i],j) * c[j]

    y = y + np.random.normal(loc=0.0, scale=amount_of_noise * np.std(y), size=y.shape)
    return  x,y


    # x = np.random.uniform(-1, 1, N)
    # y = np.zeros(N)
    # coefficients = np.random.uniform(-10, 10, size=degree + 1)
